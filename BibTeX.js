...
function writeField(field, value, isMacro, escape) {
	...
	if (!isMacro && !(field == "url" || field == "doi" || field == "file" || field == "lccn")) {
		...
		value = escapeToBibTeX(value, escape);
...
function escapeToBibTeX(value, escape) {
	if(escape.HTML) {
		//convert the HTML markup allowed in Zotero for rich text to TeX
		value = mapHTMLmarkup(value, function(value) { escapeChars(value, escape) });
	} else {
		value = escapeChars(value, escape);
	}

	return value;
}

function escapeChars(value, escape) {
	if(escape.illegal) {
...
function mapHTMLmarkup(characters, escapeCallback) {
	//convert string to DOM
	var dom = (new DOMParser()).parseFromString(characters, 'text/html');
	return DOMtoTeX(dom.body, escapeCallback);
}
...
function DOMtoTeX(element, escapeCallback) {
	var str = "";
	...
	do {
		...
		if(nodeName == '#text') {
			str += ( typeof(escapeCallback) == 'function' ? escapeCallback(node.textContent) : node.textContent );
			continue;
		}
		...
		if(typeof(escapeCallback) == "function") {
			openingTag = escapeCallback(openingTag);
			closingTag = escapeCallback(closingTag);
		}
		...
		if(item.creators && item.creators.length) {
			...
			for(var i in item.creators) {
				...
				if (creator.firstName) {
					creatorString = escapeToBibTeX(creator.lastName + ", " + creator.firstName, escape);
				} else if (creator.fieldMode == true) { // fieldMode true, assume corporate author
					creatorString = "{" + escapeToBibTeX(creator.lastName, escape) + "}";
				}
			...
			escape.HTML = false;
			escape.illegal = false;

			if(author) {
				writeField("author", author.substr(5), escape);
			}
			if(editor) {
				writeField("editor", editor.substr(5), escape);
			}
			if(translator) {
				writeField("translator", translator.substr(5), escape);
			}
		}
