...
/**
 * ReadBuffer object
 * object for reading input streams with ability to push back and peek
 */
...
//read a single line
ReadBuffer.prototype.readLine = function() {
	var lfIndex = this._buffer.indexOf('\n');
	var lineEnd, line;
	//find first LF (with optional CR)
	if(lfIndex != -1) {
		lineEnd = lfIndex;
		if(lfIndex != 0 && this._buffer[lfIndex-1] == '\r') {
			lfIndex--;
		}
	}

	//if no LF, look for CR
	if(lfIndex == -1) lfIndex = this._buffer.indexOf('\r');
	
	if(lfIndex != -1) {
		line = this._buffer.substring(0,lfIndex);
		this._buffer = this._buffer.substring(lineEnd+1);
		return line;
	}

	//if no CR or LF, get a new line from Zotero.read() and prepend whatever we had in the buffer
	line = Zotero.read();
	if(line === false && !this._buffer) return false;

	if(!line) line = '';

	line = this._buffer + line;
	this._buffer = '';

	return line;
};

//read chars number of bytes
ReadBuffer.prototype.read = function(chars) {
	if(chars === undefined) return this.readLine();

	var str;
	if(chars < this._buffer.length) {
		str = this._buffer.substring(0, chars);
		this._buffer = this._buffer.substring(chars);
		return str;
	}
	...
	if(chars) {
		var moreStr = Zotero.read(chars);
		if(moreStr === false && str === '') return false;
		str += moreStr || '';
	}
...
//push a string back onto the buffer
ReadBuffer.prototype.push = function(str) {
	this._buffer = str + this._buffer;
};

//push a back a line without having to add /r/n
//note that we can't be sure if the original line ended in /r, /n, or /r/n
ReadBuffer.prototype.pushLine = function(str) {
	this.push(str + '\r\n');
};

//peek into the next chars number of bytes without advancing the index
/**TODO: this can probably be more efficient**/
ReadBuffer.prototype.peek = function(chars) {
	var str = this.readLine(chars);

	if(str === false) return false;

	if(chars === undefined) this.pushLine(str);
	else this.push(str);
...
var fileReader = new ReadBuffer();

/** BEGIN: Utility functions **/

//consume all characters up to the first bibtex entry or the number of specified characters
function clearComments(maxChars) {
	var readAtOnce = 4096;
	var read = 0;
	if(maxChars && maxChars < readAtOnce) readAtOnce = maxChars;
	var buffer = '';
	var str, entryStart = -1;
	var re = /^[ \t]*@[a-zA-Z]+[\(\{]/m;
	while(readAtOnce && (str = fileReader.read(readAtOnce)) !== false) {
		buffer += str;
		read += str.length;

		if(buffer.indexOf('@') == -1 || (entryStart = buffer.search(re)) == -1) {
			buffer = buffer.substring(buffer.lastIndexOf('\n')+1)
										.substring(buffer.lastIndexOf('\r')+1);
		} else {
			break;
		}

		if(maxChars && maxChars < (read + readAtOnce)) {
			readAtOnce = maxChars - read;	//this will end up 0 if we reach maxChars
		}
	}

	//no entry found
	if(entryStart == -1) {
		if(str === false) {	//nothing left to read
			if(maxChars) return maxChars;
			else return read;
		} else {	//we reached maxChars. we need to push back the last line
			fileReader.push(buffer);
			return maxChars;
		}
	}
		
	buffer = buffer.substring(entryStart).replace(/^[^@]+/,'');
	fileReader.push(buffer);

	return read - buffer.length;	//return bytes consumed
}
...
function detectImport() {
	var maxChars = 1024 * 1024; // 1MB
	
	if(clearComments(maxChars) != maxChars) {
		return true;
	}
}
...
var whitespace = ' \t\r\n';
var strings;
var placeholderRE = /[a-za-Z0-9][a-zA-Z_0-9]*/;

function consumeWhiteSpace() {
	var c = fileReader.read(1);
	while(c && whitespace.indexOf(c) != -1) {
		c = fileReader.read(1);
	}
	if(c !== false) fileReader.push(c);
}

function getValue(field, type, openingChar) {
	consumeWhiteSpace();
	var oc = fileReader.read(1);	//opening character for a field
	if(!oc) return false;
	if(placeholderRE.test(oc)) {	//this means we're either reading an integer or a string reference
		fileReader.push(oc);
		oc = false;
	} else if(oc != '"' && oc != '{') {
	...
	var c = fileReader.read(1);
	var value = new Value(), part;
	var str = ''
	var breakWhile = false;
	while(c) {
		if(!oc) {
			if(!placeholderRE.test(c)) {	//end of placeholder
				fileReader.push(c);
				break;
			}
			str += c;
		} else if(c == balance[oc] || c == balance[openingChar]) {
			fileReader.push(c);
			break;
		} else {
			part = nextValuePart(field, [balance[openingChar], balance[oc]]);
			if(!part) break;
			value.addPart(part);
		}
		c = fileReader.read(1);
	...
	if(!oc) {
		if(str.search(/^[0-9]+$/) != -1) {	//integer
			value.addLiteral(str);
		} else if(str = strings[str.toLowerCase()]) {
			value.addLiteral(str);
		}
	}
	...
	consumeWhiteSpace();
	c = fileReader.read(1);
	if(c == '#') {	//concatenate string
		value.concat(getValue(field, type, openingChar));
	} else if(c) {
		fileReader.push(c);	//put it back
	}
...
function processEntry() {
	var buffer = fileReader.readLine();
	if(buffer === false) return;

	//get item type
	var type = buffer.match(/^[A-Za-z]+/);	//@ has already been consumed
	if(!type) return;

	buffer = buffer.substring(type[0].length).replace(/^\s+/,'');
	type = type[0].toLowerCase();
	if(type == 'comment' || type == 'preamble') return;

	var zoteroType = (type == 'string' ? type : bibtex2zoteroTypeMap[type]);
	if (!zoteroType) {
		Zotero.debug("Discarded item from BibTeX; type was "+type);
		return;
	}

	//get opening brace (does this have to be on the same line? It currently does)
	var openingChar = buffer[0];
	if(openingChar != '{' || openingChar != '(') return;

	//next is either the key or a field name (for strings). We allow empty keys, but not empty field names
	var m = buffer.match(/^.\s*([-A-Za-z0-9_]*)\s*([,=])\s*/);
	if(!m) return;

	buffer = buffer.substring(m[0].length);

	if(m[2] == '=') {
		//this has to be a string declaration
		if(type != 'string') return;

		fileReader.pushLine(buffer);
		var value = getValue(m[1], type, openingChar);
		if(value != false) strings[m[1].toLowerCase()] = value;
		return;
	}
	...
	entry._key = m[1];
	entry._type = type;
	entry._zType = zoteroType;

	fileReader.pushLine(buffer);

	//read field value pairs until we reach closing brace
	consumeWhiteSpace();
	var c = fileReader.peek(1), value;
	while(c && c != balance[openingChar]) {
		//get field name
		buffer = fileReader.readLine();
		m = buffer.match(/^([-A-Za-z0-9_]*)\s*=\s*/);
		if(!m) break;
		
		fileReader.pushLine(buffer.substring(m[0].length));
		
		//get value
		value = getValue(m[1], type, openingChar);
	...
	if(c == balance[openingChar]) fileReader.read(1);	//consume closing brace
...
function doImport() {
	strings = {};
	clearComments();
	var at = fileReader.read(1);

	while(at == '@') {
		processEntry();
		clearComments();
		at = fileReader.read(1);
	}
}
...
/** BEGIN: for configuring BibTeX from other translators **/
